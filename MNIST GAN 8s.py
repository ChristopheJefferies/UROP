...
index = np.nonzero(mnist.train.labels==digit)
images = mnist.train.images[index]
#labels = mnist.train.labels[index] # Not stricly needed for a GAN
...
with tf.Session() as sess:
    ...
    for e in range(epochs):
        ...
        for ii in range(len(index[0])//batch_size):
            batch_images = images[ii*batch_size : ii*batch_size + batch_size].reshape((batch_size, 784)) # Reshape images
...
# Show images as network was training, every 10 epochs. A bit broken
#rows, cols = 10, 6
