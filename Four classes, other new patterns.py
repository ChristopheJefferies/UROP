...
class0, class1 = 0, 1  # Initial image and target adversarial label

# Produces 8x8 image (as a list). Index 0 means colour in top left quadrant, 1 in top right, 2 in bottom left, 3 in bottom right
def makeimage(index):
    if index == 0: # Make horizontal lines
        builtlist = [4*[1, 1, 0, 0] + 4*[0, 0, 1, 1] for _ in range(4)]
    if index == 1: # Make vertical lines
        builtlist = [4*[0, 0, 1, 1] + 4*[1, 1, 0, 0] for _ in range(4)]
    if index == 2: # Make one checkerboard
        builtlist = [4*[1, 0] + 4*[0, 1] for _ in range(4)]
    if index == 3: # Make another checkerboard
        builtlist = [4*[0, 1] + 4*[1, 0] for _ in range(4)]
    builtlist = [item for sublist in builtlist for item in sublist] # unpack list of lists
    return [a*b for a,b in zip(builtlist, [np.random.uniform(0.7, 1) for _ in range(64)])] # add noise and return

n_train_images = 100 # Of each class
n_test_images = 10 # Of each class

# Make train data
trainindex = n_train_images*[0] + n_train_images*[1] + n_train_images*[2] + n_train_images*[3] #indices, 100 of each type
...
# Make test data
testindex = n_test_images*[0] + n_test_images*[1] + n_test_images*[2] + n_test_images*[3]
...
for i in range(4):
    image = np.array(makeimage(i)).reshape(8, 8)
    grid[i].imshow(image)
    grid[i].set_title('Label: {0}'.format(i))
plt.show()
...
# Train model
n_epochs = 40
batch_size = 50 #dataset has 200 images
...
y_train = n_train_images*[class0] + n_train_images*[class1]
np.random.shuffle(y_train)
x_train = [makeimage(i) for i in y_train]

# Make image and label for adversarial generation
chosenimage = np.array(makeimage(class0)).reshape((1, 64))
chosenlabel = to_categorical(class1, num_classes = 4) # adversarial label 0

x_train_and_adversarial = x_train + list(chosenimage.reshape(1, 64))
y_train.append('adversarial')

n_steps = 5000
lr = 1
fadefactor = 0.1 # How much to penalise brightness in adversarial image (hopefully reduces noise)

# To generate adversarial images
def create_plot_adversarial_images(x_image, y_label, lr=0.1, n_steps=5):
    original_image = x_image

    # Calculate loss, derivative and create adversarial image
    loss =  tf.nn.softmax_cross_entropy_with_logits(labels=y_label, logits=y_conv) # Change this to penalise brightness throughout the image?
    deriv = tf.gradients(loss + fadefactor*tf.reduce_mean(x), x)
    image_adv = tf.stop_gradient(x - tf.sign(deriv)*lr/n_steps) # Prevents backprop. We're treating the network as a constant, the image as changing
    ...
    for i in range(n_steps):
        ...
        x_train_and_adversarial.append(list(x_image.reshape(64,)))
        y_train.append('adversarial')

        # Occasionally plot adversarial gradient and updated image
        if (i%(n_steps//5) == 0) or (i == n_steps-1):
            plot_predictions(img_adv_list, plot_number = i/(n_steps//5) + 2)
#    return adversarial_line

# Plot adversarial image, generate line
adversarial_line = create_plot_adversarial_images(chosenimage, chosenlabel, lr=lr, n_steps=n_steps)
