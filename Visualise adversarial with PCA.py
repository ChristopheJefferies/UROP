...
# Make train data
trainindex = [n_train_images*[i] for i in range(4)] #indices, 100 of each type
...
# Make test data
testindex = [n_test_images*[i] for i in range(4)]
...
# Make two-class data in list form, ready to add adversarial images
trainindex = n_train_images*[class0] + n_train_images*[class1] # Better to pick out the images generated above with the right indices?
np.random.shuffle(trainindex)
x_train = [makeimage(i) for i in trainindex] #make images according to indices
y_train = trainindex #redundant as no longer need this to be a numpy array here
